...
while k<=length(varargin)
    if strcmpi(varargin{k},'useMultithresh')
        useMultithresh = true;
        k=k+1;
    elseif strcmpi(varargin{k},'ellipse')
        useEllipse = true;
        k = k+1;
    end
end
...
if useEllipse
    N = 4*ceil(localMaximumRadius);
    [x,y] = meshgrid(1:N);
    r1 = 2*localMaximumRadius;
    r2 = localMaximumRadius;
    
    localMaxMask = sqrt((x - N/2).^2 / r1^2 + (y - N/2).^2 / r2^2);
    localMaxMask = double(localMaxMask < 1);
    localMaxMask(round(N/2),round(N/2)) = 0;
end
...
if ~useMultithresh
    try
        [v,C] = voronoin([x_maxima y_maxima]);
    catch
        ...
        [v,C] = voronoin([x_maxima y_maxima],{'Qbb','Qz'});
    end
